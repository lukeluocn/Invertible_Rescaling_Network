...
def calculate_weights_indices(in_length, out_length, scale, kernel, kernel_width, antialiasing):
    ...
    # indices = left.reshape(out_length, 1).expand(out_length, P) + np.linspace(0, P - 1, P).reshape(
        # 1, P).expand(out_length, P)
    indices_1 = left.reshape(out_length, 1)
    indices_1 = np.tile(indices_1,(1,P))
    indices_2 = np.linspace(0, P - 1, P).reshape(1, P)
...
def imresize(img, scale, antialiasing=True):
    ...
    img_aug = np.ones((in_C, in_H + sym_len_Hs + sym_len_He, in_W)).astype(np.float32)
    ...
    inv_idx = np.arange(sym_patch.shape[1] - 1, -1, -1).astype(np.int64)
    sym_patch_inv = sym_patch[:, inv_idx]
    img_aug[:, sym_len_Hs + in_H:sym_len_Hs + in_H + sym_len_He] = sym_patch_inv.copy()
    ...
    # process W dimension
    # symmetric copying
    out_1_aug = np.ones((in_C, out_H, in_W + sym_len_Ws + sym_len_We)).astype(np.float32)
    ...
    inv_idx = np.arange(sym_patch.shape[2] - 1, -1, -1).astype(np.int64)
    sym_patch_inv = sym_patch[:,:,inv_idx]
    out_1_aug[:,:, sym_len_Ws + in_W : sym_len_Ws + in_W + sym_len_We] = sym_patch_inv.copy()
...
def imresize_np(img, scale, antialiasing=True):
    ...
    # process H dimension
    # symmetric copying
    img_aug = np.ones((in_H + sym_len_Hs + sym_len_He, in_W, in_C)).astype(np.float32)
    img_aug[sym_len_Hs:sym_len_Hs+in_H] = img.copy()
    # print(img_aug.shape)
    sym_patch = img[:sym_len_Hs, :, :]
    ...
    for i in range(out_H):
        idx = int(indices_H[i][0])
        out_1[i, :, 0] = np.matmul( img_aug[idx:idx + kernel_width, :, 0].transpose(1, 0), weights_H[i])
        out_1[i, :, 1] = np.matmul( img_aug[idx:idx + kernel_width, :, 1].transpose(1, 0), weights_H[i])
        out_1[i, :, 2] = np.matmul( img_aug[idx:idx + kernel_width, :, 2].transpose(1, 0), weights_H[i])

    # process W dimension
    # symmetric copying
    out_1_aug = np.ones((out_H, in_W + sym_len_Ws + sym_len_We, in_C)).astype(np.float32)
    out_1_aug[:,sym_len_Ws:sym_len_Ws+in_W] = out_1.copy()
    ...
    for i in range(out_W):
        idx = int(indices_W[i][0])
        out_2[:, i, 0] = np.matmul( out_1_aug[:, idx:idx + kernel_width, 0], weights_W[i])
        out_2[:, i, 1] = np.matmul( out_1_aug[:, idx:idx + kernel_width, 1], weights_W[i])
        out_2[:, i, 2] = np.matmul( out_1_aug[:, idx:idx + kernel_width, 2], weights_W[i])
...
if __name__ == '__main__':
    ...
    img = img * 1.0 / 255
    img = np.transpose(img[:, :, [2, 1, 0]], (2, 0, 1)).astype(np.float32)
    # imresize
    scale = 1 / 4
    import time
    total_time = 0
