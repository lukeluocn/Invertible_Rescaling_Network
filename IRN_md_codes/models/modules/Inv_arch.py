...
class InvBlockExp(nn.Cell):
    ...
    def construct(self, x, rev=False):
        ...
        if not rev:
            y1 = x1 + self.F(x2)
            self.s = self.clamp * (self.sigmoid(self.H(y1)) * 2 - 1)
            y2 = self.mul(x2, self.exp(self.s)) + self.G(y1)
        else:
            self.s = self.clamp * (self.sigmoid(self.H(x1)) * 2 - 1)
            y2 = self.div((x2 - self.G(x1)),self.exp(self.s))
            y1 = x1 - self.F(y2)
...
class HaarDownsampling(nn.Cell):
    def __init__(self, channel_in):
        ...
        self.conv2d_transpose = nn.Conv2dTranspose(in_channels=self.haar_weights.shape[0],
        ...
        self.conv2d_transpose.weight.requires_grad=False
